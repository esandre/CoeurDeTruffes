...
class Application_Model_About_Cgv_Multilang extends Application_Model_About_Cgv {
    ...
    protected function _refresh() {
        try {
            ...
            if (empty($this->_date)) {
                throw new Application_Model_Exception
                        ("{$this->_entityName} Date empty for {$this->_entityName} set {$this->_setId}");
            }

            $availableTranslates = $this->_translateTable->getByCgvId($this->_setId);
    ...
    public static function fetchAll($overridesActiveField) {
        $table = new Application_Model_DbTable_About_Cgv();
     ...
     * @return Application_Model_About_Cgv_Multilang
     * @throws Application_Model_Exception
     */
    public static function getLatest(){
        try{
            $cgvSet = new Application_Model_DbTable_About_Cgv();
            $id = $cgvSet->getLatestId();

            $model =  new self($id);
            if(!$model instanceof self){
                throw new Application_Model_Exception("Can't create a Cgv Model Object");
            } else {
                return $model;
            }
        } catch (Zend_Exception $e) {
            throw new Application_Model_Exception("Can't get latest Cgvs", null, $e);
        }
    }
